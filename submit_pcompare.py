"""Submit the four-way piece-count-control comparison to SLURM.

Every mechanism for controlling how many pieces a sculpture ends up with, run
against each other on the same pairs from the same starting point. Four arms,
identical in every other respect:

    nopen    lambda_count 0. SRD with no piece-count term at all: rewrites are
             accepted on image loss alone and the count is whatever growth
             happens to leave behind. The baseline the other three are
             measured against.
    penalty  lambda_count 1e-2 held fixed, the knee of the earlier lambda
             sweep. A rewrite must beat the loss by more than 1e-2 per piece
             it adds, at every point in the run.
    prune    lambda_count 1e-4 (weak enough not to bite) plus --prune-path:
             grow freely, then delete the least-damaging piece repeatedly and
             keep the fewest pieces still within --prune-tolerance of the best
             mean IoU on the path.
    hinge    The piece-count objective: rewrites scored by
                 J = n_patches + count_lambda * softplus_b(0.90 - mean_iou)^2
             so pieces *are* the objective and IoU is a constraint that stops
             costing anything once it is met. The softplus (beta 200, i.e. a
             smoothing width of about 0.005 IoU) replaces the hard hinge's
             kink, so the penalty is differentiable through the threshold
             instead of switching off discontinuously; at beta 200 the leak
             above the threshold is under 0.03 pieces.

The first three price a piece in *loss* units, which is why they are hard to
set: a piece is worth what it buys, and what it buys differs per shape. The
fourth prices a piece in pieces and states the requirement directly.

All four start from **5 pieces** rather than the usual 20, so what each
mechanism does is visible from the growth side as well as the shedding side --
with a 20-piece start a control that merely refuses to grow looks the same as
one that prunes well.
...
geometry) -- x four arms x one seed = 40 jobs, under ``results/pcompare/``.
...
sum of 4.5 (-> 3.0/1.5). Runs stop when both mean IoU and loss plateau, never
before --min-steps (1000) and never past --steps (4000) or --max-hours, and
each exports its two final camera views.

``run_final.py`` records step, mean IoU, piece count and the running
add/split/delete counts at every evaluation in ``history.csv``, so the
comparison is over whole trajectories, not just endpoints. ``--collect``
reduces each arm to what it actually delivered: pieces kept and the IoU held
at that count.

Examples
--------
    # Preview what would be submitted (writes scripts, submits nothing)
    python submit_pcompare.py --dry-run

    # Submit 10 pairs x 4 arms x 1 seed = 40 jobs
    python submit_pcompare.py

    # After jobs finish: aggregate into pcompare/<name>/collected/
    python submit_pcompare.py --sweep-name <name> --collect
"""
...
PCOMPARE_ROOT = _PROJECT_ROOT / "results" / "pcompare"
...
# How each arm controls the piece count. Same 'srd' method underneath.
ARM_SETTINGS: dict[str, dict] = {
    "nopen": {"lambda_count": 0.0, "prune_path": False, "count_objective": False},
    "penalty": {"lambda_count": 1e-2, "prune_path": False, "count_objective": False},
    "prune": {"lambda_count": 1e-4, "prune_path": True, "count_objective": False},
    "hinge": {"lambda_count": 0.0, "prune_path": False, "count_objective": True},
}
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Submit the four-way piece-count-control comparison to SLURM.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--sweep-name",
        default=None,
        help="Sweep directory under results/pcompare/ (default: pc_<timestamp>).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Starting piece count, deliberately low so growth is visible.",
    )
    ...
    hinge = parser.add_argument_group("hinge arm")
    hinge.add_argument("--count-target-iou", type=float, default=0.90)
    hinge.add_argument(
        "--count-lambda",
        type=float,
        default=2000.0,
    ...
    hinge.add_argument("--count-power", type=float, default=2.0)
    hinge.add_argument(
        "--count-softplus-beta",
        type=float,
        default=200.0,
        help="Softplus sharpness replacing the hard kink; 200 smooths over "
             "about 0.005 IoU and leaks under 0.03 pieces above the threshold.",
    )
...
_SUMMARY_KEYS = (
    "lambda_count", "count_objective", "count_lambda", "count_softplus_beta",
    "n_patches", "seed", "overlap_mode", "stop_reason",
    ...
    "iou_0p80_steps", "iou_0p85_steps", "iou_0p90_steps", "total_seconds",
...
def _pair_and_arm(job_name: str) -> tuple[str, str]:
    """Split 'pc_<pair>_<arm>' back into its parts."""
    stripped = re.sub(r"^pc_", "", job_name)
    for arm in ARMS:
...
def _collect(sweep_dir: Path) -> None:
    ...
    for report in reports:
        ...
        # One column per headline number so the arms are directly comparable:
        # on the prune arm the sculpture that ships is the pruned one, on the
        # other three it is the run's own final configuration.
        row["kept_patches"] = (
    ...
    # --- every metric over steps, every job
    ...
    mean_columns = [
        ...
        "final_mean_coverage", "srd_total_adds", "srd_total_splits",
        "srd_total_deletes", "iou_0p85_steps", "iou_0p90_steps",
    ...
    # --- console view
    print(f"{'job':<34} {'pair':<16} {'arm':<8} {'kept':>5} {'kept_iou':>9} "
          f"{'f_pc':>5} {'max_pc':>7} {'f_iou':>8} {'adds':>6} {'splits':>7} "
          f"{'dels':>6} {'step':>6} {'stop':<10}")
    print("-" * 142)
    for row in rows:
        print(
            f"{row['job']:<34} {row['pair']:<16} {row['arm']:<8} "
    ...
    print(f"{'group':<7} {'arm':<8} {'n':>3} {'kept_pc':>8} {'kept_iou':>9} "
          f"{'f_iou':>8} {'max_pc':>7} {'0p90_steps':>11} {'step':>7} {'seconds':>10}")
    for line in lines[1:]:
        fields = line.split("\t")
        ...
        print(
            ...
            f"{values['max_patches']:>7} {values['iou_0p90_steps']:>11} "
            f"{values['final_step']:>7} {values['total_seconds']:>10}"
        )
...
def main() -> None:
    ...
    sweep_dir = PCOMPARE_ROOT / sweep_name
    ...
    silhouette, negative_space = _weights(args.weight_ratio, args.weight_sum)
    print(
        f"[Sweep] pcompare/{sweep_name}: {len(jobs)} job(s) "
    ...
    for arm in args.arms:
        ...
        if settings["count_objective"]:
            detail = (
                f"J = n + {args.count_lambda:g} * softplus_{args.count_softplus_beta:g}"
                f"({args.count_target_iou:g} - iou)^{args.count_power:g}"
            )
        elif settings["prune_path"]:
            detail = (
                f"lambda_count={settings['lambda_count']:g} + pruning path "
                f"(tolerance {args.prune_tolerance:g})"
            )
        else:
            detail = f"lambda_count={settings['lambda_count']:g}"
        print(f"[Sweep]   {arm:<8} {detail}")
    print(
        f"[Sweep] base arm={BASE_ARM}, starting pieces={args.n_patches}, "
    ...
    if not args.dry_run:
        ...
        print(f"Aggregate when done: python submit_pcompare.py "
